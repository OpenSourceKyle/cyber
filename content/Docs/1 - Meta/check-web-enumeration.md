...
4. Run subdomain/vhost brute force discovery (run multiple wordlists).
    ...
    - [vHost Brute Force]({{% ref "http.md" %}})
5. Crawl the webpage for all links.
    
    - Web Crawling / Robots
    - [Crawling with FinalRecon]({{% ref "http.md" %}})
...
16. If the webserver appears to be populating data from a database, test for [SQL Injection]({{% ref "sql-injection.md" %}}).
    ...
    - [Test SQL Injection on login pages to try and bypass them]({{% ref "sql-injection.md" %}})
17. If Accounts, Pages, or other things on the webpage seem sequential and accessible in a GET or POST request, check for IDOR vulnerabilities.
...
2. [Windows Reverse Shells]({{% ref "shells.md" %}})
3. [Web Reverse Shells]({{% ref "shells.md" %}})
